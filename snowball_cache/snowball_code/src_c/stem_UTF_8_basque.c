...
static int r_aditzak(struct SN_env * z) {
    ...
    among_var = find_among_b(z, a_0, 109); /* substring, line 48 */
    ...
    switch(among_var) {
        ...
        case 1:
            {   int ret = r_RV(z);
                if (ret == 0) return 0; /* call RV, line 59 */
                if (ret < 0) return ret;
            }
        ...
        case 2:
            {   int ret = r_R2(z);
                if (ret == 0) return 0; /* call R2, line 61 */
                if (ret < 0) return ret;
            }
...
static int r_izenak(struct SN_env * z) {
    ...
    among_var = find_among_b(z, a_1, 295); /* substring, line 73 */
    if (!(among_var)) return 0;
    z->bra = z->c; /* ], line 73 */
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   int ret = r_RV(z);
                if (ret == 0) return 0; /* call RV, line 103 */
                if (ret < 0) return ret;
            }
...
static int r_adjetiboak(struct SN_env * z) {
    ...
    among_var = find_among_b(z, a_2, 19); /* substring, line 126 */
    if (!(among_var)) return 0;
    z->bra = z->c; /* ], line 126 */
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   int ret = r_RV(z);
                if (ret == 0) return 0; /* call RV, line 129 */
                if (ret < 0) return ret;
            }
...
extern int basque_UTF_8_stem(struct SN_env * z) {
    {   int c1 = z->c; /* do, line 138 */
        {   int ret = r_mark_regions(z);
            if (ret == 0) goto lab0; /* call mark_regions, line 138 */
            if (ret < 0) return ret;
        }
    lab0:
        z->c = c1;
    ...
    while(1) { /* repeat, line 140 */
        int m2 = z->l - z->c; (void)m2;
        {   int ret = r_aditzak(z);
            if (ret == 0) goto lab1; /* call aditzak, line 140 */
            if (ret < 0) return ret;
        }
        continue;
    lab1:
    ...
    while(1) { /* repeat, line 141 */
        ...
        {   int ret = r_izenak(z);
            if (ret == 0) goto lab2; /* call izenak, line 141 */
            if (ret < 0) return ret;
        }
        continue;
    lab2:
    ...
    {   int m4 = z->l - z->c; (void)m4; /* do, line 142 */
        {   int ret = r_adjetiboak(z);
            if (ret == 0) goto lab3; /* call adjetiboak, line 142 */
            if (ret < 0) return ret;
        }
    lab3:
        z->c = z->l - m4;
